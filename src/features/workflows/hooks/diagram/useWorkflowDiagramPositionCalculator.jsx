...
export default function useWorkflowDiagramPositionCalculator(id) {
    ...
    return useMemo(() => {
        ...
        workflowDiagram.initialInputIds.forEach((initialInputId, index) => {
            ...
            const upperSiblingY = positionsByDiagramId[prevInputId]?.yEnd ||
              OUTPUT_EDGE_LENGTH + WORKFLOW_START_MARGIN_TOP;
        ...
        for (let flowIndex = 0; flowIndex < workflowDiagram.flowsCount; flowIndex += 1) {
            ...
            workflowDiagram.workflowSteps.forEach((wfStep, wfStepIndex) => {
                ...
                const initialFlowY = positionsByDiagramId[flow.id]?.y || 0;
                const currentFlowY = (prevFlowYEnd || upperFlowSiblingYEnd) + FLOW_STEP_SIZE;

                const flowY = Math.max(initialFlowY, currentFlowY);
                ...
                positionsByDiagramId[flow.id] ||= flowPosition;
